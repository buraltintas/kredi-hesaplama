...
function RequestPage({ requestId }) {
  ...
  useEffect(() => {
    document.title = "Kredi Talebi | Bankacı";
  ...
  if (loading) return <main className={styles.center}><div className={styles.spinner} /><p>Talep formu hazırlanıyor…</p></main>;
  if (error && !link) return <main className={styles.center}><div className={styles.logo}>🏦</div><h1>Bağlantı kullanılamıyor</h1><p>{error}</p></main>;
  if (submitted) return <main className={styles.center}><div className={styles.success}>✓</div><h1>Talebiniz iletildi</h1><p>{link.banker.displayName}, talebiniz hakkında sizinle iletişime geçebilir.</p>{whatsapp && <a className={styles.whatsapp} href={whatsapp} target="_blank" rel="noreferrer">WhatsApp’tan görüş</a>}</main>;

  return <main className={styles.page}>
    <section className={styles.hero}>
      <div className={styles.brand}>BANKACI</div>
      <h1>{link.label}</h1>
      <div className={styles.banker}>
        ...
        <div><strong>{link.banker.displayName}</strong><span>{[link.banker.bankName, link.banker.jobTitle].filter(Boolean).join(" · ") || "Bankacı"}</span></div>
      </div>
    ...
    <form className={styles.form} onSubmit={submit}>
      <div className={styles.notice}><strong>Bilgilendirme</strong><p>Bu formdaki iletişim ve talep bilgileriniz yukarıda görünen bankacıya iletilir. Bankacı sizinle talebiniz hakkında iletişime geçebilir. Bankacı, Bankacı uygulamasından bağımsız hareket eden veri sorumlusudur.</p></div>
      ...
      <label className={styles.consent}><input type="checkbox" checked={form.consent} onChange={(e) => update("consent", e.target.checked)} /><span>Girdiğim bilgilerin ve eklediğim belgelerin talep değerlendirmesi ve benimle iletişim kurulması amacıyla bu bankacıya iletilmesini kabul ediyorum.</span></label>
      {error && <div className={styles.error}>{error}</div>}
      <button disabled={busy} type="submit">{busy ? "Talebiniz iletiliyor…" : "Talebi bankacıya ilet"}</button>
